...
import { AlertTriangle, Loader2, Radio, RefreshCw, WifiOff } from "lucide-react";
...
> = {
  ...
  reconnecting: { label: "Reconnecting", dot: "bg-amber-400", text: "text-amber-500", ring: "ring-amber-500/30" },
...
export function ConnectionBadge({
  ...
  const subtitle =
    (isError || status === "disconnected") && error
      ? error
      : isReconnecting && error
        ? error
        : isReconnecting && reconnectAttempt > 0
          ? `Attempt ${reconnectAttempt} · ${topic}`
          : topic;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col leading-tight">
        ...
        <span className="max-w-[220px] truncate font-mono text-[11px] text-ink-faint" title={subtitle}>
          {subtitle}
        </span>
      </div>
    </div>
